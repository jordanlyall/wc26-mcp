...
## 🔴 Fork additions — live results & real standings

> Fork of [`jordanlyall/wc26-mcp`](https://github.com/jordanlyall/wc26-mcp). Upstream ships static, embedded data; this fork keeps all of it and adds a **free live-results layer** on top — no paid API tier required.

**What's added:**

- **Real match results & live status** — `get_matches` overlays actual scores and a `live` / `completed` status onto the schedule, so you can see which matches are playing right now.
- **Real group tables** — `get_standings` returns a results-based `table` (points → goal difference → goals for) computed from played matches, alongside the original pre-tournament power ranking.
- **All 48 teams confirmed** — the 6 playoff slots are resolved to their real qualifiers: Czech Republic, Bosnia & Herzegovina, Turkey, Sweden, Iraq, DR Congo.

**Two free data sources, selected at runtime:**

| Source | When | Provides |
|---|---|---|
| [football-data.org](https://www.football-data.org) | `FOOTBALL_DATA_KEY` set | Full WC2026 coverage with live `IN_PLAY` status. Free tier, ~10 req/min. |
| [openfootball/worldcup.json](https://github.com/openfootball/worldcup.json) | fallback (no key) | Public-domain final scores, committed post-match. No key, no rate limit. |

Both are **fail-safe** — if the source is unreachable the server falls back to the static schedule and never errors. Enable live status by adding a [free football-data.org token](https://www.football-data.org/client/register):
...
```json
{
  "mcpServers": {
    "wc26": {
      "command": "npx",
      "args": ["-y", "wc26-mcp"],
      "env": { "FOOTBALL_DATA_KEY": "your-free-football-data-org-token" }
    }
  }
}
```

Without the key it behaves exactly like upstream, plus post-match results from openfootball.
...
| `get_matches` | Query matches with enriched team names, flags, venue details, and timezone conversion — plus real scores and live status when a results source is available *(fork)* | `date`, `date_from`, `date_to`, `team`, `group`, `venue`, `round`, `status`, `timezone` |
...
| `get_standings` | Real results-based group tables when matches have been played, plus pre-tournament power rankings (FIFA rankings, betting odds, predictions) *(fork: live table)* | `group` |
...
| Teams | 48 | All confirmed (fork resolved the 6 playoff qualifiers) |
